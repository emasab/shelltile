...
const Ext = function Ext(){
	...
	self._wsmgr =    global.workspace_manager;
	...
	self.on_remove_workspace = function(screen, index) {
		Mainloop.idle_add(Lang.bind(this, function () {
		    ...
		    for(let k in self.workspaces){
			    ...
			    var found = false;
			    for(let i=0; i<self._wsmgr.get_n_workspaces();i++){
				    var meta_workspace = self._wsmgr.get_workspace_by_index(i);
	...
	self._init_workspaces = function() {
		...
		self.connect_and_track(self, self._wsmgr, 'workspace-added', function(screen, i) { _init_workspace(i); });
